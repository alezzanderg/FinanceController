...
def add_record(is_expense=True):
    ...
    if date and category and amount:
        ...
        with open("finance_data.txt", "a") as file:
            file.write(f"{date},{category},{amount},{record_type}\n")
        status_label.config(text=f"{record_type} added successfully!", fg="green")
        if is_expense:
...
def delete_record():
    ...
    if selected_item:
        ...
        with open("finance_data.txt", "w") as file:
            for line in lines:
                if line.strip() != ",".join(item_text):
                    file.write(line)
        status_label.config(text="Record deleted successfully!", fg="green")
        update_view()
    else:
        status_label.config(text="Please select a record to delete!", fg="red")
...
def update_view():
    ...
    if os.path.exists("finance_data.txt"):
        with open("finance_data.txt", "r") as file:
            for line in file:
                date, category, amount, record_type = line.strip().split(",")
...
# Check if the 'finance_data.txt' file exists; create it if it doesn't
if not os.path.exists("finance_data.txt"):
    with open("finance_data.txt", "w"):
        pass
